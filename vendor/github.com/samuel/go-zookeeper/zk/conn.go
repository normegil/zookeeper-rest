...
func (c *Conn) connect() error {
	...
	for {
		...
		if err == nil {
			...
			c.logger.Printf("Connected to %s (%+v, %+v)", c.Server(), c.conn.LocalAddr(), c.conn.RemoteAddr())
			return nil
		}
...
func (c *Conn) loop() {
	for {
		...
		case err == ErrSessionExpired:
			c.logger.Printf("Authentication failed (Session Expired): %s", err)
...
func (c *Conn) authenticate() error {
	...
	n, err := encodePacket(buf[4:], request)
	if err != nil {
		return err
	}
