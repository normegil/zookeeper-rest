...
func TestAvailable(t *testing.T) {
	ip := "127.0.0.1"
	port := 50900
	available := tools.TCPPortAvalaible(&net.TCPAddr{
		IP:   net.ParseIP(ip),
		Port: port,
	})
	if !available {
...
func TestNotAvailable(t *testing.T) {
	...
	available := tools.TCPPortAvalaible(tcpAddr)
	if available {
...
func TestSelectPort_SameIP(t *testing.T) {
	ports := tools.Test_ParseIntervalInteger(t, "[50900;50900]")
	...
	for _, test := range tests {
		ip := net.ParseIP(test)
		addr := tools.SelectPort(ip, *ports)
...
func TestSelectPort(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			ports := tools.Test_ParseIntervalInteger(t, test.portsInterval)
			addr := tools.SelectPortExcluding(ip, *ports, test.portsExcluded)
