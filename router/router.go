...
func (r *Router) Listen(port int) error {
	userDAO := &dao.MongoUserDAO{r.Env.Session(), ""}
	handler := middleware.URLContructor(middleware.RequestLogger(r.Env.Log(), middleware.RequestAuthenticator(r.Env.Log(), userDAO, r.router)))
